...
async function run() {
    try {
        ...
        const groupName = "Flexy\\DEMO";
        console.log(`Fetching RAW configurations and overrides for group "${groupName}"...`);
        const groupRes = await makeRequest(`/api/group/get?group=${encodeURIComponent(groupName)}`);
        
        if (groupRes && groupRes.retcode === "0 Done" && groupRes.answer) {
            console.log("\n========================================================");
            console.log(`            RAW GROUP CONFIGURATION FOR "${groupName}"`);
            console.log("========================================================");
            
            // Print the core details and the Symbols override array specifically
            const g = groupRes.answer;
            console.log(JSON.stringify({
                Group: g.Group,
                LeverageMax: g.LeverageMax,
                Symbols: g.Symbols || []
            }, null, 2));
            
            console.log("========================================================\n");
        } else {
            console.error("❌ Failed to fetch configurations for group:", groupRes);
        }
