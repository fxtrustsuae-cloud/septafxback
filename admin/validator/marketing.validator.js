...
module.exports.addMarketingMember = async (request, response, next) => {
    const rules = Joi.object().keys({
        ...
        // netDeposit: Joi.number().required(),
        role: Joi.string().valid("MANAGER", "MARKETING").required(),
...
module.exports.updateMarketingMember = async (request, response, next) => {
    const rules = Joi.object().keys({
        ...
        netDeposit: Joi.number().optional(),
        role: Joi.string().valid("MANAGER", "MARKETING"),
...
module.exports.assignManager = async (request, response, next) => {
    const rules = Joi.object().keys({
        marketingId: Joi.number().min(1).required(),
...
module.exports.list = (request, response, next) => {
    const rules = Joi.object().keys({
        ...
        role: Joi.string().valid("MANAGER", "MARKETING").optional()
    });
...
module.exports.userAssignTo = (request, response, next) => {
    const rules = Joi.object().keys({
        marketingMemberId: Joi.number().min(1).required(),
        userIds: Joi.array().items(Joi.number().min(1)).min(1).required()
            .messages({
                "array.base": "userIds must be an array of numbers",
                "array.min": "userIds must contain at least 1 user ID",
            })
    });
    const { error } = rules.validate(request.body);
...
module.exports.marketingUserAssingList = (request, response, next) => {
    const rules = Joi.object().keys({
        marketingMemberId: Joi.number().integer().min(1).required(),
