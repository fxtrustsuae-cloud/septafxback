...
module.exports.moveMt5User = (request, response, next) => {
  const rules = Joi.object().keys({
    userId: Joi.number().required(),
    login: Joi.string().required()
  });
...
module.exports.requestedMt5List = (request, response, next) => {
  const rules = Joi.object().keys({
      page: Joi.number().integer().optional(),
      sizePerPage: Joi.number().integer().optional(),
      accountType: Joi.string().valid("DEMO", "REAL").optional(),
...
module.exports.approveRejectRequestedMt5 = (request, response, next) => {
  const rules = Joi.object().keys({
    mt5RequestedId: Joi.number().integer().required(),
